...
#if defined(NDEBUG)
# error "Monkcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x64cd1bb73f9bffe55f1f3991d2cfddeadfce3600d97eb4bd02e953e0e91c313f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Monkcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Monkcoin Signed Message:\n";
...
{
    // Monkcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Monkcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Monkcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Monkcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Monkcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Monkcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x41c21bfc162f22c8f8784f02fb24e173e36fc0d3e1ff7a22b0b0f359c4807ca7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN 14/Feb/2018, Parkland Highschool shooting";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f87c59b01beeaf73e9abfd8f5e4cdf8515b522749127c29dc714861e07a1b53ab4b3b82013710f7e6f91b81927683da719ff89d0e6e1b5a609cb7c7b63cbe792") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518657164;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085955707;

        if (fTestNet)
        {
            block.nTime    = 1518657137;
            block.nNonce   = 389282162;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0be88923229037a8d2c659a8158822d1e43bf6e88abfc3e5cfb9bf61270ee726"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Monkcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MonkcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MonkcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MonkcoinMiner(CWallet *pwallet)
{
    printf("MonkcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("monkcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MonkcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MonkcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MonkcoinMiner, pwallet));
}
